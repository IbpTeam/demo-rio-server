...
function PrivateKey(rsa) {
	...
	function saveKeys(priKeyPath,pubKeyPath){
		fs.appendFile(priKeyPath, rsa.getPrivateKeyPem(), 'utf8',function(err){  
        if(err)  
            console.log("fail " + err);  
        else  
            console.log("写入文件ok");  
		});  
		fs.appendFile(pubKeyPath, rsa.getPublicKeyPem(), 'utf8',function(err){  
			if(err)  
				console.log("fail " + err);  
			else  
            console.log("写入文件ok");  
    ...
    self.getPublicKeyPem = getPublicKeyPem;
    return self;
}
